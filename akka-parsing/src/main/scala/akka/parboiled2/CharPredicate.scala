...
sealed abstract class CharPredicate extends (Char ⇒ Boolean) {
  ...
  def asMaskBased: MaskBased = this match {
    case x: MaskBased ⇒ x
...
object CharPredicate {
  ...
  def from(predicate: Char ⇒ Boolean): CharPredicate = predicate match {
  ...
  object ApplyMagnet {
    implicit def fromPredicate(predicate: Char ⇒ Boolean): ApplyMagnet =
      new ApplyMagnet(from(predicate))
    ...
    implicit def fromChars(chars: Seq[Char]): ApplyMagnet = chars match {
  ...
  case class MaskBased private[CharPredicate](lowMask: Long, highMask: Long)
    ...
    def size: Int =
      java.lang.Long.bitCount(lowMask) + java.lang.Long.bitCount(highMask)
  ...
  class RangeBased private[CharPredicate](
    ...
    def ++(other: Seq[Char]): CharPredicate =
      if (other.nonEmpty) this ++ CharPredicate(other) else this
  ...
  class ArrayBased private[CharPredicate](private val chars: Array[Char])
    ...
    def ++(other: Seq[Char]): CharPredicate =
      if (other.nonEmpty)
        new ArrayBased((this -- other).chars ++ other.toArray[Char])
      else this
  ...
  case class General private[CharPredicate](predicate: Char ⇒ Boolean)
    ...
    override def toString(): String =
      "CharPredicate.General@" + System.identityHashCode(this)
  }
}
