...
class HttpExt(private val config: Config)(implicit val system: ActorSystem)
  ...
  private[this] val defaultConnectionPoolSettings = ConnectionPoolSettings(
      system)
  ...
  def bindAndHandle(
    ...
    def handleOneConnection(
        incomingConnection: IncomingConnection): Future[Done] =
  ...
  private[this] val poolMasterActorRef =
    system.actorOf(PoolMasterActor.props, "pool-master")
   ...
   * use the `akka.http.host-connection-pool` config section or pass in a [[ConnectionPoolSettings]] explicitly.
   */
  def newHostConnectionPoolHttps[T](
      host: String,
      port: Int = 443,
      connectionContext: HttpsConnectionContext = defaultClientHttpsContext,
      settings: ConnectionPoolSettings = defaultConnectionPoolSettings,
  ...
  private[akka] def newHostConnectionPool[T](
    ...
    val gateway = new PoolGateway(
        poolMasterActorRef, setup, PoolGateway.newUniqueGatewayIdentifier)
  ...
  def superPool[T](
    ...
    clientFlow[T](settings) { request ⇒
      request -> sharedGateway(request, settings, connectionContext, log)
    }
   ...
   */
  def defaultServerHttpContext: ConnectionContext = synchronized {
   ...
   */
  def defaultClientHttpsContext: HttpsConnectionContext = synchronized {
  ...
  private def sharedGateway(request: HttpRequest,
                            settings: ConnectionPoolSettings,
                            connectionContext: ConnectionContext,
                            log: LoggingAdapter): PoolGateway = {
  ...
  private def sharedGateway(hcps: HostConnectionPoolSetup): PoolGateway =
    new PoolGateway(poolMasterActorRef, hcps, PoolGateway.SharedGateway)(
        systemMaterializer)

  private def gatewayClientFlow[T](
      hcps: HostConnectionPoolSetup, gateway: PoolGateway)(
  ...
  private def clientFlow[T](settings: ConnectionPoolSettings)(
      f: HttpRequest ⇒ (HttpRequest, PoolGateway))(
    ...
    Flow[(HttpRequest, T)].mapAsyncUnordered(parallelism) {
      case (request, userContext) ⇒
        ...
        gateway(effectiveRequest).onComplete(responseTry ⇒
  ...
  private[http] def sslTlsStage(connectionContext: ConnectionContext,
                                role: TLSRole,
...
object Http extends ExtensionId[HttpExt] with ExtensionIdProvider {
   ...
   * }}}
   */
  type WebSocketClientLayer = BidiFlow[Message,
                                       SslTlsOutbound,
                                       SslTlsInbound,
                                       Message,
  ...
  final case class HostConnectionPool private[http](
      setup: HostConnectionPoolSetup)(private[http] val gateway: PoolGateway) {
    // enable test access
...
trait DefaultSSLContextCreation {
  ...
  def validateAndWarnAboutLooseSettings() = {
    ...
    if (sslConfig.config.loose.disableHostnameVerification)
      log.warning(
          "Detected that Hostname Verification is disabled globally (via ssl-config's akka.ssl-config.loose.disableHostnameVerification) for the Http extension! " +
          WarningAboutGlobalLoose)

    if (sslConfig.config.loose.disableSNI) {
      log.warning(
          "Detected that Server Name Indication (SNI) is disabled globally (via ssl-config's akka.ssl-config.loose.disableSNI) for the Http extension! " +
          WarningAboutGlobalLoose)
    }
  }
  // --- end of log warnings ---
  ...
  def createServerHttpsContext(
      sslConfig: AkkaSSLConfig): HttpsConnectionContext = {
    log.warning(
        "Automatic server-side configuration is not supported yet, will attempt to use client-side settings. " +
        "Instead it is recommended to construct the Servers HttpsConnectionContext manually (via SSLContext).")
    createClientHttpsContext(sslConfig)
  }

  def createClientHttpsContext(
      sslConfig: AkkaSSLConfig): HttpsConnectionContext = {
