...
final case class HttpCookie(name: String,
  ...
  /** Java API */
  def withExpires(dateTime: jm.DateTime): headers.HttpCookie =
    copy(expires = Some(dateTime.asScala))

  /** Java API */
  def withDomain(domain: String): headers.HttpCookie =
  ...
  /** Java API */
  def withMaxAge(maxAge: Long): headers.HttpCookie =
  ...
  /** Java API */
  def withHttpOnly(httpOnly: Boolean): headers.HttpCookie =
