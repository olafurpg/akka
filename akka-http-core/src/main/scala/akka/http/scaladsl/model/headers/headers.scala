...
private[headers] sealed trait RequestHeader extends ModeledHeader {
  override def renderInRequests = true
}
private[headers] sealed trait ResponseHeader extends ModeledHeader {
  override def renderInResponses = true
}
private[headers] sealed trait RequestResponseHeader
    extends RequestHeader with ResponseHeader
...
abstract class ModeledCustomHeaderCompanion[H <: ModeledCustomHeader[H]] {
  ...
  def apply(value: String): H = parse(value) match {
    case Success(parsed) ⇒ parsed
    case Failure(ex) ⇒
      throw new IllegalArgumentException(
          s"Unable to construct custom header by parsing: '$value'", ex)
  }
  ...
  final implicit val implicitlyLocatableCompanion: ModeledCustomHeaderCompanion[
      H] = this
}
...
final case class Accept(mediaRanges: immutable.Seq[MediaRange])
    extends jm.headers.Accept with RequestHeader {
...
object `Accept-Charset` extends ModeledCompanion[`Accept-Charset`] {
  def apply(
      first: HttpCharsetRange, more: HttpCharsetRange*): `Accept-Charset` =
...
final case class `Accept-Charset`(
    ...
    extends jm.headers.AcceptCharset with RequestHeader {
...
final case class `Accept-Encoding`(encodings: immutable.Seq[HttpEncodingRange])
    extends jm.headers.AcceptEncoding with RequestHeader {
...
final case class `Accept-Language`(languages: immutable.Seq[LanguageRange])
    extends jm.headers.AcceptLanguage with RequestHeader {
...
final case class `Accept-Ranges`(rangeUnits: immutable.Seq[RangeUnit])
    extends jm.headers.AcceptRanges with ResponseHeader {
...
object `Access-Control-Allow-Credentials`
    extends ModeledCompanion[`Access-Control-Allow-Credentials`]
...
object `Access-Control-Allow-Headers`
    extends ModeledCompanion[`Access-Control-Allow-Headers`] {
...
object `Access-Control-Allow-Origin`
   ...
   */
  def forRange(range: HttpOriginRange) =
    new `Access-Control-Allow-Origin`(range)
}
...
object `Access-Control-Expose-Headers`
    extends ModeledCompanion[`Access-Control-Expose-Headers`] {
  def apply(headers: String*): `Access-Control-Expose-Headers` =
    apply(immutable.Seq(headers: _*))
  implicit val headersRenderer = Renderer.defaultSeqRenderer[String] // cache
}
final case class `Access-Control-Expose-Headers`(
    headers: immutable.Seq[String])
...
object `Access-Control-Max-Age`
    extends ModeledCompanion[`Access-Control-Max-Age`]
final case class `Access-Control-Max-Age`(deltaSeconds: Long)
    extends jm.headers.AccessControlMaxAge with ResponseHeader {
...
object `Access-Control-Request-Headers`
    extends ModeledCompanion[`Access-Control-Request-Headers`] {
  def apply(headers: String*): `Access-Control-Request-Headers` =
    apply(immutable.Seq(headers: _*))
  implicit val headersRenderer = Renderer.defaultSeqRenderer[String] // cache
}
final case class `Access-Control-Request-Headers`(
    headers: immutable.Seq[String])
...
object `Access-Control-Request-Method`
    extends ModeledCompanion[`Access-Control-Request-Method`]
final case class `Access-Control-Request-Method`(method: HttpMethod)
    extends jm.headers.AccessControlRequestMethod with RequestHeader {
...
final case class Age(deltaSeconds: Long)
    extends jm.headers.Age with ResponseHeader {
  def renderValue[R <: Rendering](r: R): r.type = r ~~ deltaSeconds
...
final case class Allow(methods: immutable.Seq[HttpMethod])
    extends jm.headers.Allow with ResponseHeader {
...
final case class Authorization(credentials: HttpCredentials)
    extends jm.headers.Authorization with RequestHeader {
...
final case class `Cache-Control`(directives: immutable.Seq[CacheDirective])
    extends jm.headers.CacheControl with RequestResponseHeader {
...
object Connection extends ModeledCompanion[Connection] {
  def apply(first: String, more: String*): Connection =
    apply(immutable.Seq(first +: more: _*))
  implicit val tokensRenderer = Renderer.defaultSeqRenderer[String] // cache
}
final case class Connection(tokens: immutable.Seq[String])
 ...
 */
final case class `Content-Length` private[http](length: Long)
    extends RequestResponseHeader {
...
final case class `Content-Disposition`(dispositionType: ContentDispositionType,
...
final case class `Content-Encoding`(encodings: immutable.Seq[HttpEncoding])
    extends jm.headers.ContentEncoding with RequestResponseHeader {
...
object `Content-Range` extends ModeledCompanion[`Content-Range`] {
  def apply(byteContentRange: ByteContentRange): `Content-Range` =
    apply(RangeUnits.Bytes, byteContentRange)
}
final case class `Content-Range`(
    rangeUnit: RangeUnit, contentRange: ContentRange)
    extends jm.headers.ContentRange with ResponseHeader {
  def renderValue[R <: Rendering](r: R): r.type =
 ...
 */
final case class `Content-Type` private[http](contentType: ContentType)
    extends jm.headers.ContentType with RequestResponseHeader {
...
object Cookie extends ModeledCompanion[Cookie] {
  def apply(first: HttpCookiePair, more: HttpCookiePair*): Cookie =
    apply(immutable.Seq(first +: more: _*))
...
final case class Cookie(cookies: immutable.Seq[HttpCookiePair])
    extends jm.headers.Cookie with RequestHeader {
...
final case class Date(date: DateTime)
    extends jm.headers.Date with RequestResponseHeader {
  def renderValue[R <: Rendering](r: R): r.type =
    date.renderRfc1123DateTimeString(r)
...
object ETag extends ModeledCompanion[ETag] {
  def apply(tag: String, weak: Boolean = false): ETag =
    ETag(EntityTag(tag, weak))
}
final case class ETag(etag: EntityTag)
    extends jm.headers.ETag with ResponseHeader {
...
final case class Expires(date: DateTime)
    extends jm.headers.Expires with ResponseHeader {
  def renderValue[R <: Rendering](r: R): r.type =
    date.renderRfc1123DateTimeString(r)
...
object Host extends ModeledCompanion[Host] {
  def apply(authority: Uri.Authority): Host =
    apply(authority.host, authority.port)
  def apply(address: InetSocketAddress): Host =
    apply(address.getHostString, address.getPort)
  def apply(host: String): Host            = apply(host, 0)
  def apply(host: String, port: Int): Host = apply(Uri.Host(host), port)
  val empty = Host("")
}
final case class Host(host: Uri.Host, port: Int = 0)
    extends jm.headers.Host with RequestHeader {
...
final case class `If-Match`(m: EntityTagRange)
    extends jm.headers.IfMatch with RequestHeader {
...
final case class `If-Modified-Since`(date: DateTime)
    extends jm.headers.IfModifiedSince with RequestHeader {
  def renderValue[R <: Rendering](r: R): r.type =
    date.renderRfc1123DateTimeString(r)
...
final case class `If-None-Match`(m: EntityTagRange)
    extends jm.headers.IfNoneMatch with RequestHeader {
  def renderValue[R <: Rendering](r: R): r.type = r ~~ m
...
final case class `If-Range`(entityTagOrDateTime: Either[EntityTag, DateTime])
    extends RequestHeader {
  def renderValue[R <: Rendering](r: R): r.type = entityTagOrDateTime match {
    case Left(tag)       ⇒ r ~~ tag
...
final case class `If-Unmodified-Since`(date: DateTime)
    extends jm.headers.IfUnmodifiedSince with RequestHeader {
  def renderValue[R <: Rendering](r: R): r.type =
    date.renderRfc1123DateTimeString(r)
...
final case class `Last-Modified`(date: DateTime)
    extends jm.headers.LastModified with ResponseHeader {
  def renderValue[R <: Rendering](r: R): r.type =
    date.renderRfc1123DateTimeString(r)
...
object Link extends ModeledCompanion[Link] {
  def apply(uri: Uri, first: LinkParam, more: LinkParam*): Link =
    apply(immutable.Seq(LinkValue(uri, first +: more: _*)))
...
final case class Link(values: immutable.Seq[LinkValue])
    extends jm.headers.Link with RequestResponseHeader {
...
final case class Location(uri: Uri)
    extends jm.headers.Location with ResponseHeader {
  def renderValue[R <: Rendering](r: R): r.type = {
...
final case class Origin(origins: immutable.Seq[HttpOrigin])
    extends jm.headers.Origin with RequestHeader {
  def renderValue[R <: Rendering](r: R): r.type =
...
final case class `Proxy-Authorization`(credentials: HttpCredentials)
    extends jm.headers.ProxyAuthorization with RequestHeader {
  def renderValue[R <: Rendering](r: R): r.type = r ~~ credentials
...
object Range extends ModeledCompanion[Range] {
  def apply(first: ByteRange, more: ByteRange*): Range =
    apply(immutable.Seq(first +: more: _*))
  def apply(ranges: immutable.Seq[ByteRange]): Range =
    Range(RangeUnits.Bytes, ranges)
  implicit val rangesRenderer = Renderer.defaultSeqRenderer[ByteRange] // cache
}
final case class Range(rangeUnit: RangeUnit, ranges: immutable.Seq[ByteRange])
    extends jm.headers.Range with RequestHeader {
...
final case class RawHeader(name: String, value: String)
...
final case class `Raw-Request-URI`(uri: String)
    extends jm.headers.RawRequestURI with SyntheticHeader {
...
final case class `Remote-Address`(address: RemoteAddress)
    extends jm.headers.RemoteAddress with SyntheticHeader {
...
final case class Referer(uri: Uri)
    extends jm.headers.Referer with RequestHeader {
  require(
      uri.fragment.isEmpty, "Referer header URI must not contain a fragment")
...
private[http] object `Sec-WebSocket-Accept`
    extends ModeledCompanion[`Sec-WebSocket-Accept`] {
 ...
 * INTERNAL API
 */
private[http] final case class `Sec-WebSocket-Accept`(key: String)
 ...
 * INTERNAL API
 */
private[http] object `Sec-WebSocket-Key`
    extends ModeledCompanion[`Sec-WebSocket-Key`] {
  def apply(keyBytes: Array[Byte]): `Sec-WebSocket-Key` = {
    require(
        keyBytes.length == 16,
        s"Sec-WebSocket-Key keyBytes must have length 16 but had ${keyBytes.length}")
 ...
 * INTERNAL API
 */
private[http] final case class `Sec-WebSocket-Key`(key: String)
    extends RequestHeader {
  protected[http] def renderValue[R <: Rendering](r: R): r.type = r ~~ key
   ...
   */
  def isValid: Boolean =
    Try(Base64.rfc2045().decode(key)).toOption.exists(_.length == 16)
}
 ...
 * INTERNAL API
 */
private[http] object `Sec-WebSocket-Protocol`
    extends ModeledCompanion[`Sec-WebSocket-Protocol`] {
...
private[http] final case class `Sec-WebSocket-Protocol`(
    ...
    extends RequestResponseHeader {
  require(
      protocols.nonEmpty, "Sec-WebSocket-Protocol.protocols must not be empty")
 ...
 * INTERNAL API
 */
private[http] object `Sec-WebSocket-Version`
    extends ModeledCompanion[`Sec-WebSocket-Version`] {
...
private[http] final case class `Sec-WebSocket-Version`(
    ...
    extends RequestResponseHeader {
  require(
      versions.nonEmpty, "Sec-WebSocket-Version.versions must not be empty")
  require(
...
object Server extends ModeledCompanion[Server] {
  def apply(products: String): Server =
...
final case class `Strict-Transport-Security`(
    maxAge: Long, includeSubDomains: Boolean = false)
    extends jm.headers.StrictTransportSecurity with ResponseHeader {
  def renderValue[R <: Rendering](r: R): r.type = {
    r ~~ "max-age=" ~~ maxAge
...
final case class `Set-Cookie`(cookie: HttpCookie)
    extends jm.headers.SetCookie with ResponseHeader {
...
final case class `Timeout-Access`(
    timeoutAccess: akka.http.scaladsl.TimeoutAccess)
...
final case class `Tls-Session-Info`(session: SSLSession)
    extends jm.headers.TlsSessionInfo with SyntheticHeader
...
object `Transfer-Encoding` extends ModeledCompanion[`Transfer-Encoding`] {
  def apply(
      first: TransferEncoding, more: TransferEncoding*): `Transfer-Encoding` =
    apply(immutable.Seq(first +: more: _*))
  implicit val encodingsRenderer =
    Renderer.defaultSeqRenderer[TransferEncoding] // cache
}
final case class `Transfer-Encoding`(
    encodings: immutable.Seq[TransferEncoding])
    extends jm.headers.TransferEncoding with RequestResponseHeader {
  require(encodings.nonEmpty, "encodings must not be empty")
...
final case class Upgrade(protocols: immutable.Seq[UpgradeProtocol])
    extends RequestResponseHeader {
  ...
  def hasWebSocket: Boolean =
    protocols.exists(_.name equalsIgnoreCase "websocket")
}

// http://tools.ietf.org/html/rfc7231#section-5.5.3
object `User-Agent` extends ModeledCompanion[`User-Agent`] {
  def apply(products: String): `User-Agent` =
    apply(ProductVersion.parseMultiple(products))
...
final case class `User-Agent`(products: immutable.Seq[ProductVersion])
    extends jm.headers.UserAgent with RequestHeader {
  require(products.nonEmpty, "products must not be empty")
...
final case class `WWW-Authenticate`(challenges: immutable.Seq[HttpChallenge])
    extends jm.headers.WWWAuthenticate with ResponseHeader {
  require(challenges.nonEmpty, "challenges must not be empty")
...
final case class `X-Forwarded-For`(addresses: immutable.Seq[RemoteAddress])
    extends jm.headers.XForwardedFor with RequestHeader {
...
final case class `X-Real-Ip`(address: RemoteAddress)
    extends jm.headers.XRealIp with RequestHeader {
  def renderValue[R <: Rendering](r: R): r.type = r ~~ address
