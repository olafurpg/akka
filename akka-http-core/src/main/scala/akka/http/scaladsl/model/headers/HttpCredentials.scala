...
final case class BasicHttpCredentials(username: String, password: String)
    extends jm.headers.BasicHttpCredentials {
  val cookie = {
...
final case class OAuth2BearerToken(token: String)
    extends jm.headers.OAuth2BearerToken {
