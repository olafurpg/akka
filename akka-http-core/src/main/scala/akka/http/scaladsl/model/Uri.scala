...
sealed abstract case class Uri(scheme: String,
   ...
   */
  def query(charset: Charset = UTF8,
            mode: Uri.ParsingMode = Uri.ParsingMode.Relaxed): Query =
  ...
  def queryString(charset: Charset = UTF8): Option[String] =
    rawQueryString.map(s ⇒ decode(s, charset))
   ...
   */
  private[akka] def effectivePort: Int =
    if (authority.port != 0) authority.port else defaultPorts(scheme)

  /**
   * Returns a copy of this Uri with the given components.
   */
  def copy(scheme: String = scheme,
           authority: Authority = authority,
           path: Path = path,
           rawQueryString: Option[String] = rawQueryString,
  ...
  def withAuthority(host: String, port: Int): Uri =
    copy(authority = Authority(Host(host), port))
  ...
  def withHost(host: String): Uri =
    copy(authority = authority.copy(host = Host(host)))
...
object Uri {
  ...
  implicit def apply(input: String): Uri =
    apply(input: ParserInput, UTF8, Uri.ParsingMode.Relaxed)
  ...
  def apply(input: ParserInput): Uri =
    apply(input, UTF8, Uri.ParsingMode.Relaxed)
   ...
   * If the given string is not a valid URI the method throws an `IllegalUriException`.
   */
  def apply(input: ParserInput, mode: Uri.ParsingMode): Uri =
    apply(input, UTF8, mode)
   ...
   * If the given string is not a valid URI the method throws an `IllegalUriException`.
   */
  def parseAbsolute(input: ParserInput,
   ...
   * If the given string is not a valid URI the method throws an `IllegalUriException`.
   */
  def parseAndResolve(string: ParserInput,
                      base: Uri,
                      charset: Charset = UTF8,
                      mode: Uri.ParsingMode = Uri.ParsingMode.Relaxed): Uri =
   ...
   * If the given string is not a valid URI the method throws an `IllegalUriException`.
   */
  def parseHttpRequestTarget(
      requestTarget: ParserInput,
      charset: Charset = UTF8,
      mode: Uri.ParsingMode = Uri.ParsingMode.Relaxed): Uri =
  ...
  def httpScheme(securedConnection: Boolean = false) =
    if (securedConnection) "https" else "http"
  ...
  final case class Authority(host: Host, port: Int = 0, userinfo: String = "") {
    ...
    override def toString =
      UriRendering.AuthorityRenderer.render(new StringRendering, this).get
  }
  object Authority {
  ...
  sealed abstract class Host extends jm.Host {
    ...
    override def toString =
      UriRendering.HostRenderer.render(new StringRendering, this).get
  ...
  object Host {
    ...
    def apply(string: String,
              charset: Charset = UTF8,
              mode: Uri.ParsingMode = Uri.ParsingMode.Relaxed): Host =
    ...
    def apply(address: Inet6Address): IPv6Host =
      IPv6Host(address.getAddress, address.getHostAddress)
  }
  ...
  object IPv4Host {
    def apply(address: String): IPv4Host =
      apply(address.split('.').map(_.toInt.toByte))
  ...
  object IPv6Host {
    def apply(bytes: Array[Byte]): IPv6Host =
      Host(InetAddress.getByAddress(bytes).asInstanceOf[Inet6Address])
  ...
  object Path {
    ...
    case object Empty extends SlashOrEmpty {
      ...
      def charCount = 0
      def ::(segment: String) =
        if (segment.isEmpty) this else Segment(segment, this)
    ...
    final case class Slash(tail: Path) extends SlashOrEmpty {
      ...
      def reverseAndPrependTo(prefix: Path) =
        tail.reverseAndPrependTo(Slash(prefix))
      def startsWith(that: Path): Boolean =
        that.isEmpty || that.startsWithSlash && tail.startsWith(that.tail)
      def dropChars(count: Int): Path =
        if (count < 1) this else tail.dropChars(count - 1)
    }
    final case class Segment(head: String, tail: SlashOrEmpty) extends Path {
      ...
      def ::(segment: String) =
        if (segment.isEmpty) this else Segment(segment + head, tail)
    ...
    object ~ {
      def unapply(cons: Segment): Option[(String, Path)] =
  ...
  sealed abstract class Query
    ...
    def toMultiMap: Map[String, List[String]] = {
      @tailrec
      def append(map: Map[String, List[String]],
    ...
    override def newBuilder: mutable.Builder[(String, String), Query] =
      Query.newBuilder
    override def toString =
      UriRendering.QueryRenderer.render(new StringRendering, this).get
  }
  object Query {
    ...
    def apply(string: String): Query =
      apply(string: ParserInput, UTF8, Uri.ParsingMode.Relaxed)
    def apply(input: ParserInput,
              charset: Charset = UTF8,
              mode: Uri.ParsingMode = Uri.ParsingMode.Relaxed): Query =
      new UriParser(input, charset, mode).parseQuery()
    def apply(input: Option[String]): Query =
      apply(input, UTF8, Uri.ParsingMode.Relaxed)
    def apply(input: Option[String],
              charset: Charset,
    ...
    final case class Cons(key: String, value: String, override val tail: Query)
        extends Query {
  ...
  object ParsingMode {
    ...
    def apply(string: String): ParsingMode = string match {
      case "strict"  ⇒ Strict
      case "relaxed" ⇒ Relaxed
      case x ⇒
        throw new IllegalArgumentException(
            x + " is not a legal UriParsingMode")
    }
  }

  // http://tools.ietf.org/html/rfc3986#section-5.2.2
  private[http] def resolve(scheme: String,
                            userinfo: String,
                            host: Host,
                            port: Int,
                            path: Path,
      ...
      string.charAt(ix) match {
        case '%' ⇒
          def intValueOfHexWord(i: Int) = {
            def intValueOfHexChar(j: Int) = {
              ...
              if (HEXDIG(c)) CharUtils.hexValue(c)
                ...
                throw new IllegalArgumentException(
                    "Illegal percent-encoding at pos " + j)
            }
          ...
          if (((decodeBytes() >> 7) == 0) &&
            ...
            @tailrec def appendBytes(i: Int = 0): Unit = if (i < bytesCount) {
  ...
  private[http] def normalizeScheme(scheme: String): String = {
    @tailrec
    def verify(ix: Int = scheme.length - 1,
    ...
    @tailrec def process(input: Path, output: Path = Path.Empty): Path = {
      ...
      input match {
        ...
        case Slash(Segment(".", tail)) ⇒
          process(if (tail.isEmpty) Path./ else tail, output)
        case Slash(Segment("..", tail)) ⇒
          process(
  ...
  private[http] def fail(summary: String, detail: String = "") =
    throw IllegalUriException(summary, detail)
...
object UriRendering {
  ...
  implicit object AuthorityRenderer extends Renderer[Authority] {
    def render[R <: Rendering](r: R, value: Authority): r.type =
      renderAuthority(r, value, "", UTF8)
  }
  implicit object PathRenderer extends Renderer[Path] {
    def render[R <: Rendering](r: R, value: Path): r.type =
      renderPath(r, value, UTF8)
  }
  implicit object QueryRenderer extends Renderer[Query] {
    def render[R <: Rendering](r: R, value: Query): r.type =
      renderQuery(r, value, UTF8)
  }
  implicit object UriRenderer extends Renderer[Uri] {
    def render[R <: Rendering](r: R, value: Uri): r.type =
      renderUri(r, value, UTF8)
  }
  ...
  def renderUri[R <: Rendering](r: R, value: Uri, charset: Charset): r.type = {
    ...
    if (value.fragment.isDefined)
      encode(r ~~ '#', value.fragment.get, charset, `query-fragment-char`)
    r
  }
  ...
  def renderAuthority[R <: Rendering](r: R,
    ...
    if (authority.nonEmpty) {
      ...
      r ~~ '/' ~~ '/'
      if (!userinfo.isEmpty)
        encode(r, userinfo, charset, `userinfo-char`) ~~ '@'
      r ~~ host
  ...
  def renderPath[R <: Rendering](
      ...
      path: Path,
      charset: Charset,
      encodeFirstSegmentColons: Boolean = false): r.type = path match {
    case Path.Empty       ⇒ r
    case Path.Slash(tail) ⇒ renderPath(r ~~ '/', tail, charset)
    case Path.Segment(head, tail) ⇒
      val keep =
        if (encodeFirstSegmentColons) `pchar-base-nc` else `pchar-base`
  ...
  def renderQuery[R <: Rendering](
    ...
    @tailrec def append(q: Query): r.type = q match {
      case Query.Empty ⇒ r
  ...
  private[http] def encode(r: Rendering,
                           string: String,
                           charset: Charset,
                           keep: CharPredicate,
  ...
  private[http] def isAsciiCompatible(cs: Charset) =
    cs == UTF8 || cs == ISO88591 || cs == ASCII
}
...
object UriJavaAccessor {
  ...
  def hostApply(string: String, mode: Uri.ParsingMode): Host =
    Uri.Host(string, mode = mode)
  def hostApply(string: String, charset: Charset): Host =
    Uri.Host(string, charset = charset)
  def emptyHost: Uri.Host = Uri.Host.Empty

  def queryApply(params: Array[akka.japi.Pair[String, String]]): Uri.Query =
