...
abstract class ServerSettings private[akka]()
  ...
  override def withVerboseErrorMessages(newValue: Boolean): ServerSettings =
    self.copy(verboseErrorMessages = newValue)
  override def withResponseHeaderSizeHint(newValue: Int): ServerSettings =
  ...
  def withTimeouts(newValue: ServerSettings.Timeouts): ServerSettings =
    self.copy(timeouts = newValue)
  def withServerHeader(newValue: Option[Server]): ServerSettings =
    self.copy(serverHeader = newValue)
...
object ServerSettings extends SettingsCompanion[ServerSettings] {
  ...
  override def apply(config: Config): ServerSettings =
