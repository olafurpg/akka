...
private[http] object WebSocket {
  ...
  private object PrepareForUserHandler
      extends GraphStage[FlowShape[MessagePart, MessagePart]] {
    val in             = Inlet[MessagePart]("prepareForUserHandler.in")
    val out            = Outlet[MessagePart]("prepareForUserHandler.out")
    override val shape = FlowShape(in, out)
    override def initialAttributes: Attributes =
      Attributes.name("PrepareForUserHandler")
        ...
        override def onPush(): Unit = grab(in) match {
          case PeerClosed(code, reason) ⇒
            if (code.exists(Protocol.CloseCodes.isError))
              failStage(new PeerClosedConnectionException(code.get, reason))
            else if (inMessage)
              failStage(
          ...
          case ActivelyCloseWithCode(code, reason) ⇒
            if (code.exists(Protocol.CloseCodes.isError))
              failStage(new ProtocolException(
  ...
  private case object BypassRouter
      extends GraphStage[FanOutShape2[Output, BypassEvent, MessagePart]] {
  ...
  private case object BypassMerge
      extends GraphStage[FanInShape3[BypassEvent, AnyRef, Tick.type, AnyRef]] {
  ...
  private case object LiftCompletions
      extends GraphStage[FlowShape[FrameStart, AnyRef]] {
