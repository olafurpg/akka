...
private[http] abstract class HttpMessageParser[
  ...
  val stage: PushPullStage[SessionBytes, Output] =
    new PushPullStage[SessionBytes, Output] {
      ...
      private def handleParserOutput(
          output: Output, ctx: Context[Output]): SyncDirective = output match {
  ...
  @tailrec final def parseHeaderLines(
      ...
      headerCount: Int = 0,
      ch: Option[Connection] = None,
      clh: Option[`Content-Length`] = None,
      cth: Option[`Content-Type`] = None,
  ...
  def failMessageStart(summary: String, detail: String): StateResult =
