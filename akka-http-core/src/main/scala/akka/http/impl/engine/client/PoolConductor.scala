...
private object PoolConductor {
  ...
  private case class Busy(openRequests: Int) extends SlotState {
    ...
    override def createLogic(effectiveAttributes: Attributes) =
      new GraphStageLogic(shape) {
        ...
        def slotStateAfterDisconnect(
            slotState: SlotState, failed: Int): SlotState = slotState match {
  ...
  private class Route(slotCount: Int)
      extends GraphStage[UniformFanOutShape[SwitchCommand, RequestContext]] {

    override def initialAttributes = Attributes.name("PoolConductor.Route")

    override val shape =
      new UniformFanOutShape[SwitchCommand, RequestContext](slotCount)
