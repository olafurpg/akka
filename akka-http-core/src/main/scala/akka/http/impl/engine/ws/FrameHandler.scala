...
private[http] object FrameHandler {
  ...
  private class HandlerStage(server: Boolean)
      extends StatefulStage[FrameEventOrError, Output] {
    ...
    private class CollectingTextMessage
        extends CollectingMessageFrame(Opcode.Text) {
    ...
    private abstract class CollectingMessageFrame(expectedOpcode: Opcode)
        extends StateWithControlFrameHandling {
  ...
  final case class TextMessagePart(data: String, last: Boolean)
      extends MessageDataPart
  final case class BinaryMessagePart(data: ByteString, last: Boolean)
      extends MessageDataPart
