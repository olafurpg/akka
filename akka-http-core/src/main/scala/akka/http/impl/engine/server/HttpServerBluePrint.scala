...
private[http] object HttpServerBluePrint {
  ...
  def websocketSupport(settings: ServerSettings,
                       log: LoggingAdapter
  ): BidiFlow[ResponseRenderingOutput,
              ByteString,
              SessionBytes,
              SessionBytes,
              NotUsed] =
  ...
  def parsingRendering(settings: ServerSettings,
                       log: LoggingAdapter
  ): BidiFlow[ResponseRenderingContext,
  ...
  def requestTimeoutSupport(timeout: Duration
      ...
      HttpResponse, HttpResponse, HttpRequest, HttpRequest, NotUsed] =
    timeout match {
   ...
   * entity).
   */
  final class PrepareRequests(settings: ServerSettings)
  ...
  def parsing(
      ...
      log: LoggingAdapter): Flow[SessionBytes, RequestOutput, NotUsed] = {
    import settings._
  ...
  def rendering(settings: ServerSettings,
                log: LoggingAdapter
      ...
      new GraphStageLogic(shape) {
        ...
        setHandler(requestParsingIn, new InHandler {
          def onPush(): Unit = grab(requestParsingIn) match {
            ...
            case x: EntityStreamError
                if messageEndPending && openRequests.isEmpty =>
    ...
    def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
      new TimerGraphStageLogic(shape) {
        ...
        setHandler(fromHttp, new InHandler {
          override def onPush(): Unit = grab(fromHttp) match {
