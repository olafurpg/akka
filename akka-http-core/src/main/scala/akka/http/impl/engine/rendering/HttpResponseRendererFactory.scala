    ...
    def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
      new GraphStageLogic(shape) {
        ...
        setHandler(in, new InHandler {
          override def onPush(): Unit = render(grab(in)) match {
        ...
        def render(ctx: ResponseRenderingContext): StrictOrStreamed = {
          ...
          def renderStatusLine(): Unit = protocol match {
            case `HTTP/1.1` ⇒
          ...
          def completeResponseRendering(
              entity: ResponseEntity): StrictOrStreamed = entity match {
