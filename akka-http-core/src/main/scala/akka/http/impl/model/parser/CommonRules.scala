...
private[parser] trait CommonRules {
  ...
  def OWS =
    rule { zeroOrMore(optional(CRLF) ~ oneOrMore(WSP)) } // extended with `obs-fold`
  ...
  def `quoted-pair` =
    rule { '\\' ~ (`quotable-base` | `obs-text`) ~ appendSB() }
  ...
  def weight =
    rule { ws(';') ~ ws('q') ~ ws('=') ~ qvalue } // a bit more lenient than the spec
  ...
  def `token68` =
    rule { capture(oneOrMore(`token68-start`) ~ zeroOrMore('=')) ~ OWS }
  ...
  def `opaque-tag` =
    rule { '"' ~ capture(zeroOrMore(`etagc-base` | `obs-text`)) ~ '"' }
  ...
  def `expires-av` = rule {
    ignoreCase("expires=") ~ OWS ~ `expires-date` ~> {
  ...
  def `byte-content-range` =
    rule { `bytes-unit` ~ (`byte-range-resp` | `unsatisfied-range`) }
  ...
  def `byte-ranges-specifier` =
    rule { `bytes-unit` ~ ws('=') ~ `byte-range-set` }
  ...
  def `other-range-resp` =
    rule { capture(zeroOrMore(ANY)) ~> ContentRange.Other }
  ...
  def uriReference: Rule1[Uri] =
    rule { runSubParser(newUriParser(_).`URI-reference-pushed`) }
}
