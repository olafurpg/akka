...
private[parser] trait IpAddressParsing {
  ...
  def `ip-number` = rule {
    capture(
        '2' ~ (DIGIT04 ~ DIGIT | '5' ~ DIGIT05) | '1' ~ DIGIT ~ DIGIT | DIGIT19 ~ DIGIT | DIGIT) ~>
    (java.lang.Integer.parseInt(_).toByte)
  }
  ...
  def `ip-v6-address`: Rule1[Array[Byte]] = {
    ...
    def zero2(ix: Int) =
      rule { run { a(ix) = 0.toByte; a(ix + 1) = 0.toByte; } }
    def h4(ix: Int) = rule { HEXDIG ~ run(a(ix) = hv(lastChar).toByte) }
    def h8(ix: Int) = rule {
      HEXDIG ~ HEXDIG ~ run(
          a(ix) = (hv(charAt(-2)) * 16 + hv(lastChar)).toByte)
    }
    def h16(ix: Int) = rule {
    ...
    rule {
      ...
      (h16c(0) ~ tail2 | cc(0) ~ tail2 | ch16o(0) ~
          (cc(2) ~ tail4 | ch16o(2) ~
              (cc(4) ~ tail6 | ch16o(4) ~
                  (cc(6) ~ tail8 | ch16o(6) ~
                      (cc(8) ~ tail10 | ch16o(8) ~
                          (cc(10) ~ ls32 | ch16o(10) ~
                              (cc(12) ~ h16(14) | ch16o(12) ~ cc(14))))))))
    }
  }

  def `ip-v6-reference`: Rule1[String] =
    rule { capture('[' ~ oneOrMore(HEXDIG | anyOf(":.")) ~ ']') }
}
