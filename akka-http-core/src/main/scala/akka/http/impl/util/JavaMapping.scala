  ...
  implicit def fromJavaSeqMapping[J](implicit mapping: J2SMapping[J]
    ...
    new J2SMapping[Seq[J]] {
      ...
      def toScala(javaObject: Seq[J]): S =
        javaObject.map(mapping.toScala(_)).toList
    }
}
...
private[http] object JavaMapping {
  ...
  object Implicits {
    ...
    implicit def convertToScala[J](j: J)(
        implicit mapping: J2SMapping[J]): mapping.S = mapping.toScala(j)
    implicit def convertSeqToScala[J](j: Seq[J])(
  ...
  def identity[T]: JavaMapping[T, T] = new JavaMapping[T, T] {
    def toJava(scalaObject: T): J = scalaObject
    def toScala(javaObject: T): S = javaObject
  }

  implicit def iterableMapping[_J, _S](implicit mapping: JavaMapping[_J, _S]
      ): JavaMapping[jl.Iterable[_J], immutable.Seq[_S]] =
    new JavaMapping[jl.Iterable[_J], immutable.Seq[_S]] {
      import collection.JavaConverters._

      def toJava(scalaObject: immutable.Seq[_S]): jl.Iterable[_J] =
        scalaObject.map(mapping.toJava).asJavaCollection
      def toScala(javaObject: jl.Iterable[_J]): immutable.Seq[_S] =
        Implicits.convertSeqToScala(
            iterableAsScalaIterableConverter(javaObject).asScala.toSeq)
    }
  implicit def map[K, V]: JavaMapping[ju.Map[K, V], immutable.Map[K, V]] =
    new JavaMapping[ju.Map[K, V], immutable.Map[K, V]] {
      import scala.collection.JavaConverters._
      def toScala(javaObject: ju.Map[K, V]): immutable.Map[K, V] =
        javaObject.asScala.toMap
  ...
  implicit def option[_J, _S](implicit mapping: JavaMapping[_J, _S]
    ...
    new JavaMapping[Optional[_J], Option[_S]] {
      ...
      def toJava(scalaObject: Option[_S]): Optional[_J] =
  ...
  def scalaToJavaAdapterFlow[J, S](
      implicit mapping: JavaMapping[J, S]): scaladsl.Flow[S, J, NotUsed] =
  ...
  implicit def tryMapping[_J, _S](
      implicit mapping: JavaMapping[_J, _S]): JavaMapping[Try[_J], Try[_S]] =
  ...
  class Inherited[J <: AnyRef, S <: J](implicit classTag: ClassTag[S])
      extends JavaMapping[J, S] {
  ...
  implicit object DateTime
      extends Inherited[jm.DateTime, akka.http.scaladsl.model.DateTime]

  implicit object ContentType extends Inherited[jm.ContentType, sm.ContentType]
  implicit object ContentTypeBinary
      extends Inherited[jm.ContentType.Binary, sm.ContentType.Binary]
  ...
  implicit object ContentTypeRange
      extends Inherited[jm.ContentTypeRange, sm.ContentTypeRange]
  ...
  implicit object HttpCharsetRange
      extends Inherited[jm.HttpCharsetRange, sm.HttpCharsetRange]
  ...
  implicit object HttpProtocol
      extends Inherited[jm.HttpProtocol, sm.HttpProtocol]
  implicit object HttpRequest extends Inherited[jm.HttpRequest, sm.HttpRequest]
  implicit object HttpResponse
      extends Inherited[jm.HttpResponse, sm.HttpResponse]
  ...
  implicit object MediaTypeBinary
      extends Inherited[jm.MediaType.Binary, sm.MediaType.Binary]
  ...
  implicit object ContentRange
      extends Inherited[jm.ContentRange, sm.ContentRange]
  implicit object RemoteAddress
      extends Inherited[jm.RemoteAddress, sm.RemoteAddress]
  implicit object TransferEncoding
      extends Inherited[jm.TransferEncoding, sm.TransferEncoding]

  implicit object HostHeader
      extends Inherited[jm.headers.Host, sm.headers.Host]
  implicit object Server
      extends Inherited[jm.headers.Server, sm.headers.Server]
  implicit object ByteRange
      extends Inherited[jm.headers.ByteRange, sm.headers.ByteRange]
  implicit object CacheDirective
      extends Inherited[jm.headers.CacheDirective, sm.headers.CacheDirective]
  ...
  implicit object EntityTagRange
      extends Inherited[jm.headers.EntityTagRange, sm.headers.EntityTagRange]
  implicit object HttpChallenge
      extends Inherited[jm.headers.HttpChallenge, sm.headers.HttpChallenge]
  implicit object HttpCookie
      extends Inherited[jm.headers.HttpCookie, sm.headers.HttpCookie]
  implicit object HttpCookiePair
      extends Inherited[jm.headers.HttpCookiePair, sm.headers.HttpCookiePair]
  implicit object HttpCredentials
      extends Inherited[jm.headers.HttpCredentials, sm.headers.HttpCredentials]
  ...
  implicit object HttpOriginRange
      extends Inherited[jm.headers.HttpOriginRange, sm.headers.HttpOriginRange]
  implicit object Language
      extends Inherited[jm.headers.Language, sm.headers.Language]
  implicit object LanguageRange
      extends Inherited[jm.headers.LanguageRange, sm.headers.LanguageRange]
  implicit object LinkParam
      extends Inherited[jm.headers.LinkParam, sm.headers.LinkParam]
  implicit object LinkValue
      extends Inherited[jm.headers.LinkValue, sm.headers.LinkValue]
  implicit object ProductVersion
      extends Inherited[jm.headers.ProductVersion, sm.headers.ProductVersion]
  implicit object RangeUnit
      extends Inherited[jm.headers.RangeUnit, sm.headers.RangeUnit]
