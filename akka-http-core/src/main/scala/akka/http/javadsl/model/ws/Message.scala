...
object TextMessage {
   ...
   */
  def create(text: String): TextMessage = new TextMessage {
   ...
   */
  def create(textStream: Source[String, _]): TextMessage = new TextMessage {
...
object BinaryMessage {
   ...
   */
  def create(data: ByteString): BinaryMessage = new BinaryMessage {
    def isStrict: Boolean                      = true
  ...
  def create(dataStream: Source[ByteString, _]): BinaryMessage =
    new BinaryMessage {
      ...
      def asScala: sm.ws.BinaryMessage =
        sm.ws.BinaryMessage(dataStream.asScala)
    }
