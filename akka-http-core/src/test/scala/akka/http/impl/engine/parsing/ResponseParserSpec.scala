...
class ResponseParserSpec
  ...
  private class Test {
      ...
      override def equals(other: scala.Any): Boolean = other match {
        case other: StrictEqualHttpResponse ⇒
          this.resp.copy(entity = HttpEntity.Empty) == other.resp.copy(
              ...
              this.resp.entity.toStrict(awaitAtMost), awaitAtMost) == Await
            .result(other.resp.entity.toStrict(awaitAtMost), awaitAtMost)
      }
    ...
    def strictEqualify[T](
        x: Either[T, HttpResponse]): Either[T, StrictEqualHttpResponse] =
    ...
    def generalRawMultiParseTo(expected: Either[ResponseOutput, HttpResponse]*
        ): Matcher[Seq[String]] = generalRawMultiParseTo(GET, expected: _*)
    ...
    def rawParse(requestMethod: HttpMethod,
                 input: String*
