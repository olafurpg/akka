...
class SslConfigWarningsSpec extends WordSpec with Matchers {
  ...
  "ssl-config loose options should cause warnings to be logged" should {
    ...
    "warn if SNI is disabled globally" in {
      ...
      EventFilter.warning(
          start = "Detected that Server Name Indication (SNI) is disabled globally ",
    ...
    "warn if hostname verification is disabled globally" in {
      ...
      val msgStart =
        "Detected that Hostname Verification is disabled globally "

      EventFilter.warning(start = msgStart, occurrences = 1) intercept {
        Http()(system)
      }

      // the very big warning shall be logged only once per actor system (extension)
      EventFilter.warning(start = msgStart, occurrences = 0) intercept {
        Http()(system)
      }
