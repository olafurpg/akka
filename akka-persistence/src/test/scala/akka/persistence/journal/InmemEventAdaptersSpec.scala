...
class InmemEventAdaptersSpec extends AkkaSpec {
  ...
  val config = ConfigFactory
    .parseString(
        s"""
           ...
           |    event-adapters {
           |      example  = ${classOf[
           ExampleEventAdapter].getCanonicalName}
           |      marker   = ${classOf[
           MarkerInterfaceAdapter].getCanonicalName}
           |      precise  = ${classOf[
           PreciseAdapter].getCanonicalName}
           |      reader  = ${classOf[
           ReaderAdapter].getCanonicalName}
           |      writer  = ${classOf[
           WriterAdapter].getCanonicalName}
           |    }
           |    event-adapter-bindings = {
           |      "${classOf[
           EventMarkerInterface].getCanonicalName}" = marker
  ...
  "EventAdapters" must {
    ...
    "fail with useful message when binding to not defined adapter" in {
      ...
      val combinedConfig =
        badConfig.getConfig("akka.persistence.journal.inmem")
