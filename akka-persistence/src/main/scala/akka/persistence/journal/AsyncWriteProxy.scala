...
private[persistence] trait AsyncWriteProxy
  ...
  def asyncDeleteMessagesTo(
      persistenceId: String, toSequenceNr: Long): Future[Unit] = store match {
    case Some(s) ⇒
  ...
  def asyncReplayMessages(persistenceId: String,
                          ...
                          toSequenceNr: Long,
                          max: Long)(
      replayCallback: PersistentRepr ⇒ Unit): Future[Unit] = store match {
    case Some(s) ⇒
      ...
      s.tell(ReplayMessages(persistenceId, fromSequenceNr, toSequenceNr, max),
             mediator)
...
private[persistence] object AsyncWriteTarget {
  ...
  @SerialVersionUID(1L)
  final case class ReplayMessages(persistenceId: String,
                                  fromSequenceNr: Long,
                                  toSequenceNr: Long,
                                  max: Long)
