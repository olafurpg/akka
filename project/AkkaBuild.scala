...
object AkkaBuild extends Build {
  ...
  override def buildLoaders =
    BuildLoader.transform(Sample.buildTransformer) :: Nil
  ...
  ).settings(parentSettings: _*).aggregate(httpSprayJson, httpXml)
  ...
  ).settings(parentSettings: _*).aggregate(httpJackson)
  ...
  lazy val sampleMainJavaLambda =
    Sample.project("akka-sample-main-java-lambda")

  lazy val sampleMultiNodeScala =
    Sample.project("akka-sample-multi-node-scala")
  ...
  lazy val osgiDiningHakkersSampleMavenTest = Project(
      ...
      base = file("akka-samples/akka-sample-osgi-dining-hakkers-maven-test")
        ...
        compile in Compile <<= (publishM2 in actor,
  ...
  private def allWarnings: Boolean =
    System.getProperty("akka.allwarnings", "false").toBoolean
  ...
  lazy val defaultSettings =
    resolverSettings ++ TestExtras.Filter.settings ++ Protobuf.settings ++ Seq(
        ...
        initialCommands := """|import language.postfixOps
                             |import akka.actor._
                             |import ActorDSL._
    ...
    ) ++ mavenLocalResolverSettings ++ JUnitFileReporting.settings ++ docLintingSettings
