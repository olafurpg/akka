...
class UnfoldResourceAsyncSourceSpec extends AkkaSpec(UnboundedMailboxConfig) {
  ...
  val open: () ⇒ Future[BufferedReader] = () ⇒
    Promise
      .successful(new BufferedReader(new FileReader(manyLinesFile)))
      .future
  val read: (BufferedReader) ⇒ Future[Option[String]] = reader ⇒
    Promise.successful(Option(reader.readLine())).future
  val close: (BufferedReader) ⇒ Future[Done] = reader ⇒
    {
      reader.close()
  ...
  "Unfold Resource Async Source" must {
    "read contents from a file" in assertAllStagesStopped {
      ...
      val p = Source
        .unfoldResourceAsync[String, BufferedReader](
            () ⇒
              {
    ...
    "continue when Strategy is Resume and exception happened" in assertAllStagesStopped {
            ...
            reader ⇒
              {
                ...
                if (s != null && s.contains("b")) throw TE("")
                else Promise.successful(Option(s)).future
            },
            close)
      ...
      (0 to 49).foreach(i ⇒
          ...
          sub.request(1)
          c.expectNext() should ===(
              if (i < 10) manyLinesArray(i) else manyLinesArray(i + 10))
      })
      sub.request(1)
      c.expectComplete()
    }
    ...
    "work with ByteString as well" in assertAllStagesStopped {
      ...
      val p = Source
        .unfoldResourceAsync[ByteString, Reader](
            open,
            reader ⇒
              {
                ...
                if (s > 0)
                  p.success(Some(ByteString(buffer.mkString("")).take(s)))
                else p.success(None)
                p.future
            },
            reader ⇒
    ...
    "use dedicated blocking-io-dispatcher by default" in assertAllStagesStopped {
      ...
      try {
        val p = Source
          .unfoldResourceAsync[String, BufferedReader](open, read, close)
        ...
        val ref = expectMsgType[Children].children
          .find(_.path.toString contains "unfoldResourceSourceAsync")
          .get
