...
object CircuitBreakerProxy {
  ...
  final case class CircuitClosed(circuit: ActorRef)
      extends CircuitBreakerCommand
  final case class CircuitHalfOpen(circuit: ActorRef)
      extends CircuitBreakerCommand
  ...
  final case class CircuitBreakerPropsBuilder(
    ...
    def withCircuitEventListener(value: Option[ActorRef]) =
      copy(circuitEventListener = value)
    def withFailureDetector(value: Any ⇒ Boolean) =
      copy(failureDetector = value)
    def withOpenCircuitFailureConverter(value: CircuitOpenFailure ⇒ Any) =
      copy(openCircuitFailureConverter = value)
  ...
  when(Closed) {
    commonStateHandling orElse {
      ...
      case Event(message, state) ⇒
        log.debug(
            "CLOSED: Sending message {} expecting a response withing timeout {}",
            message,
            callTimeout)
  ...
  when(HalfOpen) {
    commonStateHandling orElse {
      ...
      case Event(CallFailed, CircuitBreakerStateData(_, true)) ⇒
        log.debug(
            "HalfOpen: First forwarded call failed returning to OPEN state")
        goto(Open)
      ...
      case Event(message, state @ CircuitBreakerStateData(_, false)) ⇒
        log.debug(
            "HalfOpen: First message {} received, forwarding it to target {}",
            message,
            target)
        forwardRequest(message, sender, state, log)
