...
private[http] trait ToNameReceptacleEnhancements {
  implicit def symbol2NR(symbol: Symbol): NameReceptacle[String] =
    new NameReceptacle[String](symbol.name)
  implicit def string2NR(string: String): NameReceptacle[String] =
    new NameReceptacle[String](string)
}
...
class NameReceptacle[T](val name: String) {
  ...
  def as[B](unmarshaller: FSU[B]) =
    new NameUnmarshallerReceptacle(name, unmarshaller)
...
class NameUnmarshallerReceptacle[T](val name: String, val um: FSU[T]) {
  ...
  def !(requiredValue: T) =
    new RequiredValueUnmarshallerReceptacle(name, requiredValue, um)
