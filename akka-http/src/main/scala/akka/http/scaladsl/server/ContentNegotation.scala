...
final class MediaTypeNegotiator(requestHeaders: Seq[HttpHeader]) {
   ...
   */
  def qValueFor(mediaType: MediaType): Float = acceptedMediaRanges match {
    case Nil ⇒ 1.0f
    case x                                           ⇒
...
final class CharsetNegotiator(requestHeaders: Seq[HttpHeader]) {
   ...
   * See http://tools.ietf.org/html/rfc7231#section-5.3.1 for details.
   */
  def qValueFor(charset: HttpCharset): Float = acceptedCharsetRanges match {
    case Nil ⇒ 1.0f
    case x                                         ⇒
   ...
   */
  def pickBest: Option[HttpCharset] = acceptedCharsetRanges match {
    case Nil                                      ⇒ Some(`UTF-8`)
...
final class ContentNegotiator(requestHeaders: Seq[HttpHeader]) {
  ...
  def qValueFor(alternative: Alternative): Float = alternative match {
...
final class EncodingNegotiator(requestHeaders: Seq[HttpHeader]) {
   ...
   * See http://tools.ietf.org/html/rfc7231#section-5.3.1 for details.
   */
  def qValueFor(encoding: HttpEncoding): Float = acceptedEncodingRanges match {
    case Nil ⇒ 1.0f
    case x                                          ⇒
...
final class LanguageNegotiator(requestHeaders: Seq[HttpHeader]) {
   ...
   * See http://tools.ietf.org/html/rfc7231#section-5.3.1 for details.
   */
  def qValueFor(language: Language): Float = acceptedLanguageRanges match {
    case Nil ⇒ 1.0f
    case x                                          ⇒
