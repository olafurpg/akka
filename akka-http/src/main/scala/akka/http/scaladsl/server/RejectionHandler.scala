...
trait RejectionHandler extends (immutable.Seq[Rejection] ⇒ Option[Route]) { self ⇒
   ...
   */
  def seal: RejectionHandler = this match {
  ...
  def default =
    newBuilder()
      .handleAll[SchemeRejection] { rejections ⇒
        ...
        complete((BadRequest,
                  "Uri scheme not allowed, supported schemes: " + schemes))
      }
      .handleAll[MethodRejection] { rejections ⇒
        val (methods, names) =
          rejections.map(r ⇒ r.supported -> r.supported.name).unzip
      ...
      .handle {
        case TooManyRangesRejection(_) ⇒
          complete((RequestedRangeNotSatisfiable,
