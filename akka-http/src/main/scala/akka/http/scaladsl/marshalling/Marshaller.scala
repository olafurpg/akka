...
object Marshaller
  ...
  def apply[A, B](f: ExecutionContext ⇒ A ⇒ Future[List[Marshalling[B]]]
      ): Marshaller[A, B] = new Marshaller[A, B] {
  ...
  def withFixedContentType[A, B](contentType: ContentType)(
      marshal: A ⇒ B): Marshaller[A, B] = strict { value ⇒
  ...
  def withOpenCharset[A, B](mediaType: MediaType.WithOpenCharset)(
      marshal: (A, HttpCharset) ⇒ B): Marshaller[A, B] = strict { value ⇒
   ...
   */
  def opaque[A, B](marshal: A ⇒ B): Marshaller[A, B] = strict { value ⇒
