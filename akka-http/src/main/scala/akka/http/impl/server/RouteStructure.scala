...
private[http] object RouteStructure {
  ...
  abstract case class FileAndResourceRouteWithDefaultResolver(
      ...
      extends Route
  case class GetFromResource(
      resourcePath: String, contentType: ContentType, classLoader: ClassLoader)
      extends Route
  case class GetFromResourceDirectory(resourceDirectory: String,
  ...
  case class PathSuffix(pathElements: immutable.Seq[PathMatcher[_]])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class PathSuffixTest(pathElements: immutable.Seq[PathMatcher[_]])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class RawPathPrefix(pathElements: immutable.Seq[PathMatcher[_]])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class RawPathPrefixTest(pathElements: immutable.Seq[PathMatcher[_]])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  ...
  case class Extract(extractions: Seq[StandaloneExtractionImpl[_]])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class BasicAuthentication(authenticator: HttpBasicAuthenticator[_])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class OAuth2Authentication(authenticator: OAuth2Authenticator[_])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class EncodeResponse(coders: immutable.Seq[Coder])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class DecodeRequest(coders: immutable.Seq[Coder])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  ...
  case class HandleExceptions(handler: ExceptionHandler)(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class HandleRejections(handler: RejectionHandler)(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  ...
  abstract class GenericHostFilter(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends HostFilter
  case class SchemeFilter(scheme: String)(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute

  case class RangeSupport()(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute

  case class HandleWebSocketMessages(handler: Flow[Message, Message, Any])
      extends Route

  case class SetCookie(cookie: HttpCookie)(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
  case class DeleteCookie(
      name: String, domain: Option[String], path: Option[String])(
      val innerRoute: Route, val moreInnerRoutes: immutable.Seq[Route])
      extends DirectiveRoute
