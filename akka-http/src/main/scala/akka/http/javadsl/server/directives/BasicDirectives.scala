...
abstract class BasicDirectives extends BasicDirectivesBase {
   ...
   */
  def complete(text: String): Route = new OpaqueRoute() {
   ...
   * A route that completes the request with a static text
   */
  def complete(response: HttpResponse): Route = new OpaqueRoute() {
   ...
   */
  def completeWithStatus(code: StatusCode): Route = new OpaqueRoute() {
