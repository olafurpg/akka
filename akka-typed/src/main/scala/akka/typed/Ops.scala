...
object Ops {
  ...
  implicit class ActorSystemOps(val sys: akka.actor.ActorSystem)
      extends AnyVal {
  ...
  implicit class ActorContextOps(val ctx: akka.actor.ActorContext)
      extends AnyVal {
    def spawn[T](props: Props[T]): ActorRef[T] =
  ...
  implicit def actorRefAdapter(ref: akka.actor.ActorRef): ActorRef[Any] =
    ActorRef(ref)
}
