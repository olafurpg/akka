...
class ReceiverSpec extends TypedSpec {
  ...
  object `A Receiver` {
    ...
    def `must reply with messages which arrived first in the same order (GetOne)`(
        ): Unit = setup("getMoreOrderOne") { (int, ext, inbox) ⇒
      ext.run(Msg(1))
      ext.run(Msg(2))
      ext.run(Msg(3))
      ext.run(Msg(4))
    ...
    def `must reply with messages which arrived first in the same order (GetAll)`(
        ): Unit = setup("getMoreOrderAll") { (int, ext, inbox) ⇒
      ext.run(Msg(1))
      ext.run(Msg(2))
    ...
    def `must reply to GetAll with messages which arrived first and later`(
        ): Unit = setup("getAllFirstAndLater") { (int, ext, inbox) ⇒
      ext.run(Msg(1))
      ext.run(Msg(2))
      int.run(GetAll(1.second)(inbox.ref))
      val msg = int.getAllEffects() match {
