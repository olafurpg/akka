...
private[akka] class BalancingDispatcher(
  ...
  private[akka] val team = new ConcurrentSkipListSet[ActorCell](
    Helpers.identityHashComparator(new Comparator[ActorCell] {
      def compare(l: ActorCell, r: ActorCell): _root_.scala.Int = l.self.path compareTo r.self.path
    }))
  ...
  override protected[akka] def dispatch(receiver: ActorCell, invocation: Envelope): _root_.scala.Unit = {
