...
final case class BalancingPool(
  ...
  def nrOfInstances(sys: ActorSystem): _root_.scala.Int = this.nrOfInstances
...
private[akka] object BalancingPoolDeploy {
  val invalidConfigKeyChars: _root_.scala.collection.immutable.List[_root_.scala.Char] = List('$', '@', ':')
}
