...
private[io] object SelectionHandler {
  ...
  private class ChannelRegistryImpl(
    ...
    def register(channel: SelectableChannel, initialOps: Int)(
        implicit channelActor: ActorRef): Unit = execute {
      new Task {
        def tryRun(): Unit =
          try {
    ...
    private abstract class Task extends Runnable {
      ...
      def run() {
        try tryRun() catch {
          ...
          case NonFatal(e) ⇒
            log.error(e, "Error during selector management task: [{}]", e)
        }
      }
    }
  }
...
private[io] class SelectionHandler(settings: SelectionHandlerSettings)
  ...
  def receive: Receive = {
    case cmd: WorkerForCommand ⇒
      spawnChildWithCapacityProtection(cmd, SelectorAssociationRetries)

    case Retry(cmd, retriesLeft) ⇒
      spawnChildWithCapacityProtection(cmd, retriesLeft)
