...
object ActorSystem {
  ...
  def apply(name: String, config: Config): ActorSystem =
    apply(name, Option(config), None, None)
  ...
  class Settings(classLoader: ClassLoader, cfg: Config, final val name: String) {
    ...
    final val config: Config = {
      val config =
        cfg.withFallback(ConfigFactory.defaultReference(classLoader))
    ...
    final val CreationTimeout: Timeout = Timeout(
        config.getMillisDuration("akka.actor.creation-timeout"))
    final val UnstartedPushTimeout: Timeout = Timeout(
        config.getMillisDuration("akka.actor.unstarted-push-timeout"))
    ...
    final val LogDeadLetters: Int = toRootLowerCase(
        config.getString("akka.log-dead-letters")) match {
      case "off" | "false" ⇒ 0
...
private[akka] class ActorSystemImpl(
   ...
   */
  protected def createDynamicAccess(): DynamicAccess =
  ...
  def actorOf(props: Props): ActorRef =
    if (guardianProps.isEmpty)
      guardian.underlying.attachChild(props, systemService = false)
    else
      throw new UnsupportedOperationException(
          "cannot create top-level actor from the outside on ActorSystem with custom user guardian")
   ...
   */
  @tailrec
  private def findExtension[T <: Extension](ext: ExtensionId[T]): T =
    ...
    findExtension(ext) match {
      case null ⇒ //Doesn't already exist, commence registration
        ...
        extensions.putIfAbsent(ext, inProcessOfRegistration) match {
          // Signal that registration is in process
          case null ⇒
            try {
        ...
        dynamicAccess.getObjectFor[AnyRef](fqcn) recoverWith {
          ...
          case Success(other) ⇒
            if (!throwOnLoadFail)
              ...
              throw new RuntimeException(
                  s"[$fqcn] is not an 'ExtensionIdProvider' or 'ExtensionId'")
          case Failure(problem) ⇒
            if (!throwOnLoadFail)
              ...
              throw new RuntimeException(
                  s"While trying to load extension [$fqcn]", problem)
        }
      }
    }
