...
object LineNumbers {
  ...
  final case class SourceFile(filename: String) extends Result {
    override def toString: _root_.scala.Predef.String = filename
  }
  final case class SourceFileLines(filename: String, from: Int, to: Int) extends Result {
    override def toString: _root_.java.lang.String = if (from != to) s"$filename:$from-$to" else s"$filename:$from"
  }
