...
trait PriorityQueueStabilizer[E <: AnyRef] extends AbstractQueue[E] {
  ...
  override def peek(): E = {
    ...
    if (wrappedElement eq null) null.asInstanceOf[E]
    else wrappedElement.element
  }
...
object PriorityQueueStabilizer {
  ...
  class WrappedElementComparator[E](val cmp: Comparator[E])
 ...
 */
class StablePriorityQueue[E <: AnyRef](capacity: Int, cmp: Comparator[E])
