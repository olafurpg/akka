...
private[akka] object SubclassifiedIndex {
  ...
  class Nonroot[K, V](override val root: SubclassifiedIndex[K, V], val key: K, _values: Set[V])(implicit sc: Subclassification[K]) extends SubclassifiedIndex[K, V](_values) {
    ...
    override def toString: _root_.scala.Predef.String = subkeys.mkString("Nonroot(" + key + ", " + values + ",\n", ",\n", ")")
  }
...
private[akka] class SubclassifiedIndex[K, V] private (protected var values: Set[V])(implicit sc: Subclassification[K]) {
  ...
  protected var subkeys: _root_.scala.collection.immutable.Vector[_root_.akka.util.SubclassifiedIndex.Nonroot[K, V]] = Vector.empty[Nonroot[K, V]]

  def this()(implicit sc: Subclassification[K]) = this(Set.empty)

  protected val root: _root_.akka.util.SubclassifiedIndex[K, V] = this
  ...
  override def toString: _root_.scala.Predef.String = subkeys.mkString("SubclassifiedIndex(" + values + ",\n", ",\n", ")")
