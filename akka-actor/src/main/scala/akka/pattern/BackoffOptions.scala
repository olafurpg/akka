...
import akka.pattern.{ BackoffOptionsImpl, BackoffReset }
...
private final case class BackoffOptionsImpl(
  ...
  def withReplyWhileStopped(replyWhileStopped: Any): BackoffOptionsImpl = copy(replyWhileStopped = Some(replyWhileStopped))
