 ...
 * when it times out.
 */
class AskTimeoutException(message: String, cause: Throwable)
...
trait ExplicitAskSupport {
  ...
  implicit def ask(actorRef: ActorRef): ExplicitlyAskableActorRef =
    new ExplicitlyAskableActorRef(actorRef)
   ...
   */
  implicit def ask(
      actorSelection: ActorSelection): ExplicitlyAskableActorSelection =
    ...
    case ref: InternalActorRef ⇒
      if (timeout.duration.length <= 0)
        Future.failed[Any](new IllegalArgumentException(
                s"""Timeout length must be positive, question not sent to [$actorRef]. Sender[$sender] sent the message of type "${message.getClass.getName}"."""))
      else {
...
final class ExplicitlyAskableActorRef(val actorRef: ActorRef) extends AnyVal {
   ...
   * INTERNAL API: for binary compatibility
   */
  private[pattern] def internalAsk(messageFactory: ActorRef ⇒ Any,
                                   timeout: Timeout,
  ...
  private[pattern] def internalAsk(
    ...
    actorSel.anchor match {
      case ref: InternalActorRef ⇒
        if (timeout.duration.length <= 0)
          Future.failed[Any](new IllegalArgumentException(
                  s"""Timeout length must be positive, question not sent to [$actorSel]. Sender[$sender] sent the message of type "${message.getClass.getName}"."""))
        else {
          val a = PromiseActorRef(ref.provider,
                                  timeout,
                                  targetName = actorSel,
...
private[akka] final class PromiseActorRef private (
  ...
  def this(provider: ActorRefProvider, result: Promise[Any]) =
