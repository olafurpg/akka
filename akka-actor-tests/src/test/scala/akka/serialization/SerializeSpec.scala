...
object SerializationTests {
  ...
  final case class Address(
      no: String, street: String, city: String, zip: String) {
    def this() = this("", "", "", "")
  }
  @BeanInfo
...
class SerializeSpec extends AkkaSpec(SerializationTests.serializeConf) {
  ...
  "Serialization" must {
    ...
    "serialize Person" in {
      assert(
          deserialize(serialize(person).get, classOf[Person]).get === person)
    }
    ...
    "resolve serializer in the order of the bindings" in {
      ser.serializerFor(classOf[A]).getClass should ===(
    ...
    "resolve serializer in the order of most specific binding first" in {
      ser.serializerFor(classOf[A]).getClass should ===(
          classOf[JavaSerializer])
      ser.serializerFor(classOf[D]).getClass should ===(
          classOf[TestSerializer])
      ser.serializerFor(classOf[E]).getClass should ===(
          classOf[TestSerializer])
    }
    ...
    "use ByteArraySerializer for byte arrays" in {
      ...
      for (a ← Seq("foo".getBytes("UTF-8"), null: Array[Byte], Array[Byte]())) byteSerializer
...
class VerifySerializabilitySpec
    extends AkkaSpec(SerializationTests.verifySerializabilityConf) {
  import SerializationTests._
...
class ReferenceSerializationSpec
    extends AkkaSpec(SerializationTests.mostlyReferenceSystem) {
  import SerializationTests._
...
class SerializationCompatibilitySpec
    extends AkkaSpec(SerializationTests.mostlyReferenceSystem) {
...
class OverriddenSystemMessageSerializationSpec
    extends AkkaSpec(SerializationTests.systemMessageMultiSerializerConf) {
  import SerializationTests._
