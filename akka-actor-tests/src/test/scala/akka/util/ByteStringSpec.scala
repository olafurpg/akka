...
class ByteStringSpec extends WordSpec with Matchers with Checkers {
  ...
  implicit val arbitraryByteStringSlice: Arbitrary[ByteStringSlice] =
    Arbitrary {
      for {
  ...
  def arbSlice[A](arbArray: Arbitrary[Array[A]]): Arbitrary[ArraySlice[A]] =
    Arbitrary {
      for {
  ...
  implicit val arbitraryLongArrayNumBytes: Arbitrary[ArrayNumBytes[Long]] =
    Arbitrary {
      for {
  ...
  implicit val arbitraryByteStringBuilder: Arbitrary[ByteStringBuilder] =
  ...
  def likeVectors(bsA: ByteString, bsB: ByteString)(
      body: (IndexedSeq[Byte], IndexedSeq[Byte]) ⇒ Any): Boolean = {
    val vecA = Vector(bsA: _*)
    val vecB = Vector(bsB: _*)
    body(bsA, bsB) == body(vecA, vecB)
  }

  def likeVecIt(bs: ByteString)(
      body: BufferedIterator[Byte] ⇒ Any, strict: Boolean = true): Boolean = {
  ...
  def likeVecIts(a: ByteString, b: ByteString)(
    ...
    val (vecAIt, vecBIt) =
      (Vector(a: _*).iterator.buffered, Vector(b: _*).iterator.buffered)
  ...
  def testShortDecoding(
      slice: ByteStringSlice, byteOrder: ByteOrder): Boolean = {
    val elemSize             = 2
    val (bytes, from, until) = slice
    val (n, a, b) =
      (bytes.length / elemSize, from / elemSize, until / elemSize)
  ...
  def testShortEncoding(
      slice: ArraySlice[Short], byteOrder: ByteOrder): Boolean = {
    val elemSize         = 2
  ...
  def testLongEncoding(
      slice: ArraySlice[Long], byteOrder: ByteOrder): Boolean = {
    val elemSize         = 8
    val (data, from, to) = slice
    val reference        = Array.ofDim[Byte](data.length * elemSize)
  ...
  def testLongPartEncoding(
      anb: ArrayNumBytes[Long], byteOrder: ByteOrder): Boolean = {
    val elemSize       = 8
    val (data, nBytes) = anb
    ...
    ByteBuffer.wrap(reference).order(byteOrder).asLongBuffer.put(data)
    val builder = ByteString.newBuilder
    for (i ← 0 until data.length) builder.putLongPart(data(i), nBytes)(
        byteOrder)

    reference.zipWithIndex
      .collect({
        // Since there is no partial put on LongBuffer, we need to collect only the interesting bytes
        case (r, i)
  ...
  def testFloatEncoding(
      slice: ArraySlice[Float], byteOrder: ByteOrder): Boolean = {
    val elemSize         = 4
    val (data, from, to) = slice
    val reference        = Array.ofDim[Byte](data.length * elemSize)
  ...
  def testDoubleEncoding(
      slice: ArraySlice[Double], byteOrder: ByteOrder): Boolean = {
    val elemSize         = 8
    val (data, from, to) = slice
    val reference        = Array.ofDim[Byte](data.length * elemSize)
    ...
    "behave as expected" when {
      ...
      "compacting" in {
        check { a: ByteString ⇒
          ...
          val b          = a.compact
          ((!wasCompact) || (b eq a)) && (b == a) && b.isCompact &&
          (b.compact eq b)
        }
      }
    ...
    "function as expected" when {
      ...
      "putting Bytes, using the OutputStream wrapper" in {
        ...
        check { slice: ArraySlice[Byte] ⇒
          ...
          for (i ← to until data.length) builder.asOutputStream.write(
              data(i).toInt)
          data.toSeq == builder.result
        }
      }
