  ...
  "The PromiseRef" must {
    "complete promise with received message" in {
      ...
      val target = system.actorOf(Props(new Actor {
        def receive: _root_.scala.PartialFunction[_root_.scala.Any, _root_.scala.Unit] = { case Request(replyTo) ⇒ replyTo ! Response }
      }))
    ...
    "work with explicitly constructed PromiseRef's" in {
      ...
      val alice = system.actorOf(Props(new Actor {
        def receive: _root_.scala.PartialFunction[_root_.scala.Any, _root_.scala.Unit] = { case Response ⇒ promise.success(42) }
      }))
