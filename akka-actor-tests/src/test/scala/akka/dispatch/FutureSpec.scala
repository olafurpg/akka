...
object FutureSpec {
  ...
  final case class IntSub(n: Int) extends IntAction {
    def apply(that: Int) = that - n
  }
  final case class IntMul(n: Int) extends IntAction {
    def apply(that: Int) = that * n
  }
  final case class IntDiv(n: Int) extends IntAction {
    def apply(that: Int) = that / n
  }
  ...
  final case class FlatMapAction(
      action: IntAction)(implicit ec: ExecutionContext)
      extends FutureAction {
    def /:(that: Try[Int]): Try[Int] = that map action.apply
    def /:(that: Future[Int]): Future[Int] =
    ...
    "being tested" must {
      ...
      "recover from exceptions" in {
        filterException[RuntimeException] {
          ...
          intercept[ArithmeticException] {
            Await.result(future6, timeout.duration)
          }
          Await.result(future7, timeout.duration) should ===("You got ERROR")
          intercept[RuntimeException] {
            Await.result(future8, timeout.duration)
          }
      ...
      "zip" in {
        ...
        intercept[IllegalStateException] {
          Await.result(Promise.failed[String](f).future zip Promise
                         .successful("foo")
                         .future,
                       timeout)
        } should ===(f)
        ...
        intercept[IllegalStateException] {
          Await.result(Promise.successful("foo").future zip Promise
                         .failed[String](f)
                         .future,
                       timeout)
        } should ===(f)
      ...
      "fold with an exception" in {
        filterException[IllegalArgumentException] {
          ...
          }.getMessage should ===("shouldFoldResultsWithException: expected")
        }
      }
      ...
      "fold mutable zeroes safely" in {
        ...
        def test(testNumber: Int) {
          ...
          val result =
            Await.result(f.mapTo[ArrayBuffer[Int]], 10000 millis).sum
      ...
      "reduce results" in {
        ...
        assert(
            Await.result(Future.reduce(futures)(_ + _), remainingOrDefault) === 55)
      }
      ...
      "reduce results with Exception" in {
        filterException[IllegalArgumentException] {
          ...
          }.getMessage should ===("shouldReduceResultsWithException: expected")
        }
      }
      ...
      "traverse Futures" in {
        ...
        assert(
            Await.result(Future.sequence(oddFutures), timeout.duration).sum === 10000)
