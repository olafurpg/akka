...
class ReplicatedDataSerializer(val system: ExtendedActorSystem)
  ...
  def gsetFromProto(gset: rd.GSet): GSet[Any] =
    GSet(
        gset.getStringElementsList.iterator.asScala.toSet ++ gset.getIntElementsList.iterator.asScala ++ gset.getLongElementsList.iterator.asScala ++ gset.getOtherElementsList.iterator.asScala
  ...
  def versionVectorFromProto(versionVector: rd.VersionVector): VersionVector = {
    ...
    if (entries.isEmpty) VersionVector.empty
      ...
      VersionVector(uniqueAddressFromProto(entries.get(0).getNode),
