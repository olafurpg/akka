...
private[akka] class SSLSettings(config: Config) {
  ...
  private def emptyIsNone(s: String): Option[String] =
    Option(s).filter(_.length > 0)
  ...
  val SSLRandomNumberGenerator = emptyIsNone(
      getString("random-number-generator"))
  ...
  if (SSLKeyStore.isDefined && SSLKeyPassword.isEmpty)
    throw new ConfigurationException(
...
private[akka] object NettySSLSupport {
   ...
   */
  def apply(settings: SSLSettings,
            log: LoggingAdapter,
