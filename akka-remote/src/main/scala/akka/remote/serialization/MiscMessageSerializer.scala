...
class MiscMessageSerializer(val system: ExtendedActorSystem)
  ...
  private def serializeActorIdentity(
      actorIdentity: ActorIdentity): Array[Byte] = {
    val builder = ContainerFormats.ActorIdentity
      .newBuilder()
  ...
  private def actorRefBuilder(
      actorRef: ActorRef): ContainerFormats.ActorRef.Builder =
  ...
  private def deserializeActorRef(
      actorRef: ContainerFormats.ActorRef): ActorRef =
