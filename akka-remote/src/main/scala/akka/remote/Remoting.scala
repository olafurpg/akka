...
private[remote] object AddressUrlEncoder {
  def apply(address: Address): String =
    URLEncoder.encode(address.toString, UTF_8)
}

/**
 * INTERNAL API
 */
private[remote] final case class RARP(provider: RemoteActorRefProvider)
    extends Extension {
  def configureDispatcher(props: Props): Props =
    provider.remoteSettings.configureDispatcher(props)
}
  ...
  private[Remoting] class TransportSupervisor
    ...
    def receive = {
      case RegisterTransportActor(props, name) ⇒
        sender() ! context.actorOf(RARP(context.system).configureDispatcher(
...
private[remote] class Remoting(
    _system: ExtendedActorSystem, _provider: RemoteActorRefProvider)
    extends RemoteTransport(_system, _provider) {

  @volatile private var endpointManager: Option[ActorRef] = None
  @volatile private var transportMapping: Map[
      String, Set[(AkkaProtocolTransport, Address)]] = _
  ...
  val transportSupervisor = system.systemActorOf(
      configureDispatcher(Props[TransportSupervisor]), "transports")
  ...
  val eventPublisher = new EventPublisher(
      system, log, RemoteLifecycleEventsLogLevel)
    ...
    endpointManager match {
      case None ⇒
        ...
        try {
          ...
          log.info("Remoting started; listening on addresses :" +
              addresses.mkString("[", ", ", "]"))
...
private[remote] object EndpointManager {
  ...
  final case class Quarantine(remoteAddress: Address, uid: Option[Int])
      extends RemotingCommand
  ...
  final case class Pass(
      endpoint: ActorRef, uid: Option[Int], refuseUid: Option[Int])
      extends EndpointPolicy {
  ...
  final case class Quarantined(uid: Int, timeOfRelease: Deadline)
      extends EndpointPolicy {
    override def isTombstone: Boolean = true
  }
  ...
  class EndpointRegistry {
    ...
    def registerWritableEndpoint(address: Address,
                                 uid: Option[Int],
                                 refuseUid: Option[Int],
                                 endpoint: ActorRef): ActorRef =
    ...
    def registerReadOnlyEndpoint(
        address: Address, endpoint: ActorRef, uid: Int): ActorRef = {
    ...
    def unregisterEndpoint(endpoint: ActorRef): Unit =
      if (isWritable(endpoint)) {
        ...
        addressToWritable.get(address) match {
          case Some(policy) if policy.isTombstone ⇒
          // There is already a tombstone directive, leave it there
    ...
    def addressForWriter(writer: ActorRef): Option[Address] =
      writableToAddress.get(writer)

    def writableEndpointWithPolicyFor(
        address: Address): Option[EndpointPolicy] =
      addressToWritable.get(address)
    ...
    def readOnlyEndpointFor(address: Address): Option[(ActorRef, Int)] =
      addressToReadonly.get(address)

    def isWritable(endpoint: ActorRef): Boolean =
      writableToAddress contains endpoint

    def isReadOnly(endpoint: ActorRef): Boolean =
      readonlyToAddress contains endpoint

    def isQuarantined(address: Address, uid: Int): Boolean =
      writableEndpointWithPolicyFor(address) match {
    ...
    def markAsQuarantined(
        address: Address, uid: Int, timeOfRelease: Deadline): Unit =
...
private[remote] class EndpointManager(conf: Config, log: LoggingAdapter)
  ...
  val eventPublisher = new EventPublisher(
      context.system, log, settings.RemoteLifecycleEventsLogLevel)
  ...
  def keepQuarantinedOr(remoteAddress: Address)(body: ⇒ Unit): Unit =
    endpoints.refuseUid(remoteAddress) match {
      case Some(uid) ⇒
        log.info(
        ...
        endpoints.markAsQuarantined(
            remoteAddress, uid, Deadline.now + settings.QuarantineDuration)
      case None ⇒ body
    }

  override val supervisorStrategy = OneForOneStrategy(loggingEnabled = false) {
    case e @ InvalidAssociation(
        localAddress, remoteAddress, reason, disassiciationInfo) ⇒
    ...
    case NonFatal(e) ⇒
      ...
      endpoints.markAsFailed(
          sender(), Deadline.now + settings.RetryGateClosedFor)
      Stop
  }
  ...
  val accepting: Receive = {
    ...
    case Quarantine(address, uidToQuarantineOption) ⇒
      ...
      (endpoints.writableEndpointWithPolicyFor(address), uidToQuarantineOption) match {
        case (Some(Pass(endpoint, _, _)), None) ⇒
          ...
          log.warning(
              ...
              settings.RetryGateClosedFor.toMillis)
          endpoints.markAsFailed(
              endpoint, Deadline.now + settings.RetryGateClosedFor)
      ...
      uidToQuarantineOption foreach { uid ⇒
        endpoints.markAsQuarantined(
            address, uid, Deadline.now + settings.QuarantineDuration)
  ...
  def handleInboundAssociation(
      ia: InboundAssociation, writerIsIdle: Boolean): Unit = ia match {
    case ia @ InboundAssociation(handle: AkkaProtocolHandle) ⇒
      endpoints.readOnlyEndpointFor(handle.remoteAddress) match {
        ...
        case None ⇒
          if (endpoints.isQuarantined(
                  handle.remoteAddress, handle.handshakeInfo.uid))
  ...
  private def createAndRegisterEndpoint(
      handle: AkkaProtocolHandle, refuseUid: Option[Int]): Unit = {
    val writing =
      settings.UsePassiveConnections &&
      !endpoints.hasWritableEndpointFor(handle.remoteAddress)
    eventPublisher.notifyListeners(AssociatedEvent(
            handle.localAddress, handle.remoteAddress, inbound = true))
    ...
    if (writing)
      endpoints.registerWritableEndpoint(handle.remoteAddress,
                                         Some(handle.handshakeInfo.uid),
                                         refuseUid,
                                         endpoint)
    else {
      endpoints.registerReadOnlyEndpoint(
          handle.remoteAddress, endpoint, handle.handshakeInfo.uid)
  ...
  private def listens: Future[Seq[(AkkaProtocolTransport, Address, Promise[
    ...
    val transports: Seq[AkkaProtocolTransport] =
      for ((fqn, adapters, config) ← settings.Transports) yield {
        ...
        val args = Seq(classOf[ExtendedActorSystem] -> context.system,
        ...
        val wrappedTransport = adapters.map {
          ...
          (t: Transport, provider: TransportAdapterProvider) ⇒
            ...
            provider.create(
                t, context.system.asInstanceOf[ExtendedActorSystem])
        }

        // Apply AkkaProtocolTransport wrapper to the end of the chain
        // The chain at this point:
        //   AkkaProtocolTransport <- Adapter <- ... <- Adapter <- Driver
        new AkkaProtocolTransport(wrappedTransport,
                                  context.system,
  ...
  private def removePendingReader(
      takingOverFrom: ActorRef, withHandle: AkkaProtocolHandle): Unit = {
  ...
  private def createEndpoint(remoteAddress: Address,
    ...
    require(transportMapping contains localAddress,
            "Transport mapping is not defined for the address")
