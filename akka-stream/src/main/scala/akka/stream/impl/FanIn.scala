...
private[akka] object FanIn {
  ...
  abstract class InputBunch(inputCount: Int, bufferSize: Int, pump: Pump) {
    ...
    override def toString: String =
      s"""|InputBunch
         |  marked:    ${states.iterator
           .map(marked(_))
           .mkString(", ")}
         |  pending:   ${states.iterator
           .map(pending(_))
           .mkString(", ")}
         |  depleted:  ${states.iterator
           .map(depleted(_))
           .mkString(", ")}
         |  completed: ${states.iterator
           .map(completed(_))
           .mkString(", ")}
         |  cancelled: ${states.iterator
           .map(cancelled(_))
           .mkString(", ")}
         |
    ...
    def cancel(): Unit = if (!allCancelled) {
      allCancelled = true
      var i = 0
    ...
    def cancel(input: Int) = if (!cancelled(input)) {
      inputs(input).cancel()
