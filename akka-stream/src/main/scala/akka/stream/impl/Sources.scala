...
private[stream] object QueueSource {
  ...
  final case class Offer[+T](elem: T, promise: Promise[QueueOfferResult])
      extends Input[T]
...
private[akka] final class SourceQueueAdapter[T](
  ...
  def watchCompletion(): CompletionStage[Done] =
    delegate.watchCompletion().toJava
...
private[stream] final class UnfoldResourceSource[T, S](
    create: () ⇒ S, readData: (S) ⇒ Option[T], close: (S) ⇒ Unit)
    extends GraphStage[SourceShape[T]] {
  val out            = Outlet[T]("UnfoldResourceSource.out")
  override val shape = SourceShape(out)
  override def initialAttributes: Attributes =
    DefaultAttributes.unfoldResourceSource
